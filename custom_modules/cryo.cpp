...
//rwh    pC->assign_position(1, 1, 1); //oocyte position
...
//   for (int i=0; i<1; i++)
   {
       std::cout << "cell " <<i<< ": "<<(*all_cells)[i]->position[0]<<","
            <<(*all_cells)[i]->position[0]<<","<<(*all_cells)[i]->position[0]<< std::endl;
    //    for(int j=0; j<3; j++)
    //    {
...
//    default_microenvironment_options.Y_range = {-250, 250};
    ...
    float delta=2.0;
    default_microenvironment_options.dx = delta;
    default_microenvironment_options.dy = delta;
    default_microenvironment_options.dz = delta;
